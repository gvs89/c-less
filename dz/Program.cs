...
class UserInputToCompileForTest
{
    ...
    public static int FindElementByPosition(int[,] array, int x, int y)
    {
        ...
        {
            Console.WriteLine($"Позиция {x},{y} выходит за пределы массива");
            return -1;
        }
    }
    ...
    {
        if (FindElementByPosition.ValidatePosition(numbers, x, y))
        {
            Console.WriteLine($"Элемент с позицией {x},{y} равен {numbers[x, y]}");
        }
        else
        {
            Console.WriteLine($"Позиция {x},{y} некорректна");
        }
    }
}
